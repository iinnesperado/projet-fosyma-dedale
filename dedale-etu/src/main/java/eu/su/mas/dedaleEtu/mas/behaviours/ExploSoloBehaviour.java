 ...
 * The algorithm is a pseudo - DFS computationally consuming because its not
 * optimised at all.
 * <p>
 * 
 * When all the nodes around him are visited, the agent randomly select an open
 * node and go there to restart its dfs.
 * <p>
...
public class ExploSoloBehaviour extends SimpleBehaviour {
	...
	public ExploSoloBehaviour(final Agent myagent, MapRepresentation myMap) {
		super(myagent);
		this.myMap = myMap;
	...
	public void action() {
		...
		if (myPosition != null) {
			...
			if (!this.myMap.hasOpenNode()) {
				...
				// capacity is:"+ ((AbstractDedaleAgent)this.myAgent).getBackPackFreeSpace());
				// System.out.println(this.myAgent.getLocalName()+" - The agent tries to
				// transfer is load into the Silo (if reachable); succes ? :
				// "+((AbstractDedaleAgent)this.myAgent).emptyMyBackPack("Silo"));
				// System.out.println(this.myAgent.getLocalName()+" - My current backpack
				// capacity is:"+ ((AbstractDedaleAgent)this.myAgent).getBackPackFreeSpace());
